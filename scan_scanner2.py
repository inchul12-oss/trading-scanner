...
■ 하드게이트 (5개, 전부 필수 — 하나라도 스킵되면 진입신호 안 냄)
  (1) 20일 이동평균(EMA) > 50일 이동평균(EMA) (단기 추세가 살아있는가, 9/4 밤 SMA→EMA 전환)
  (2) 오늘 50일선(EMA) > 5거래일 전 50일선(EMA) (추세가 유지/상승 중인가, 기울기)
...
9/4 밤 추가(SMA→EMA 전환, 인철님+형배+판호 3자 논의 후 확정): 20일선/50일선을 SMA(단순
이동평균)에서 EMA(지수이동평균)로 전환함. 모멘텀 돌파 스윙 전략 특성상 최근 가격에 더
큰 가중치를 주는 EMA가 골든크로스/추세기울기 신호를 SMA보다 1~3일 정도 더 빨리 잡아줄
것으로 판단해서 결정함. 논의 중 나온 우려 2가지는 검토 후 다음과 같이 정리함:
  - 신생상장주 EMA 예열 부족: MIN_DAILY_BARS(55)는 그대로 유지하기로 함. 50일 EMA가
    완전히 예열 안 된 상태라도, 우리가 노리는 2~4개월차 신생 모멘텀주를 원천배제하지
    않는 게 더 중요하다고 판단(9/4 낮 200일선 조건부 처리와 같은 맥락). 예열 부족은
    EMA를 실제보다 낮게 만드는 방향으로 작용해서 오히려 골든크로스가 더 쉽게 뜨는
    쪽이라, 전략 방향(모멘텀 빨리 잡기)과 상충하지 않는다고 결론.
  - 횡보장 휩소 증가: 하드게이트 5개 + 액션트리거(3개 중 2개)까지 다 통과해야 최종
    진입신호가 나가는 구조라, MA크로스 자체가 좀 더 잦아져도 실제 오신호로 이어질
    확률은 낮다고 판단.
200일선은 장기추세를 확인하는 배경필터 역할(둔감하게 잡아주는 게 목적에 맞음)이라
SMA 그대로 유지, fetch 기간(1년치)도 그대로 둠(200일 EMA를 정확히 쓰려면 예열용으로
훨씬 긴 히스토리가 필요해서 보류). 청산 로직(scan_scanner3.py)의 20일선도 전부 SMA
그대로 유지함 — 청산 쪽까지 EMA로 예민하게 만들면 어젯밤에 겨우 잡은 "돌파캔들저점"
휩소 문제가 다른 조건에서 재발할 수 있어서 의도적으로 안 건드림.
"""
import json
import time
...
def compute_daily_metrics(daily):
    ...
    # 9/4 밤 수정: 20/50일선 SMA(rolling)→EMA(ewm) 전환. adjust=False로 실시간 계산과
    # 동일한 재귀식(전일EMA + 알파*(오늘종가-전일EMA))을 쓰도록 고정.
    ma20_series = daily["Close"].ewm(span=MA_SHORT, adjust=False).mean()
    ma50_series = daily["Close"].ewm(span=MA_MID, adjust=False).mean()
